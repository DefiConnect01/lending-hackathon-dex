...
import { formatBN, formatCurrency, getTXUUID, parseBN, removeDuplicate } from "../lib/utils";
import stores from "./";
...
class Store {
  ...
  async loadBaseAssets() {
    try {
      ...
      console.log({
        baseAssets: await getBaseAssets()
      })
    } catch (e) {
  ...
  quoteSwap = async (payload) => {
    console.log({
      payload,
      routeAssets: this.getStore("routeAssets"),
      baseAssets: this.getStore("baseAssets")
    })
  ...
  merge = async (payload) => {
    await merge(
      ...
      async () => {
        await this.getNFTByID("fetchAll");
        console.log(`await this.router.push("/vest");`)
        // await this.router.push("/vest");
      }
    )
  };
  ...
  migrate = async (payload) => {
    try {
      ...
      const migratorContract = new web3.eth.Contract(
        migratorAbi,
        migrator.migratorAddress[import.meta.env.VITE_PUBLIC_CHAINID]
      );
